...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // Rebuilt Auth Guard & Profile Sync
  useEffect(() => {
    const syncUser = async () => {
      ...
      if (session?.user) {
        ...
        if (profile) {
          // If profile exists, check if name or avatar has changed and update if necessary
          if (
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen bg-bg-primary relative">
      {/* TOP NAVIGATION BAR */}
      <header className="w-full h-20 px-6 md:px-8 glass-panel border-b border-[#ECECE7]/60 flex justify-between items-center z-40 sticky top-0">
            ...
            {profileMenuOpen && (
              <>
                ...
                <div className="absolute right-0 top-12 w-80 glass-panel border border-[#DADAD4] rounded-[28px] p-5 flex flex-col gap-4 shadow-premium z-50 animate-scale-in">
                  ...
                  {/* Group 1: Optimize, My Reports, History (Background Color: Suitable Light Grey) */}
            ...
            <div className="bg-[#ECECE7]/50 border border-[#DADAD4]/40 rounded-[20px] p-2.5 flex flex-col gap-1">
              <Link
                ...
                className={`flex items-center gap-3 h-11 px-3 rounded-xl text-xs font-bold transition-all ${
                  pathname === '/dashboard' ? 'bg-accent text-white shadow-premium' : 'text-text-primary hover:bg-[#ECECE7]/60'
                }`}
              >
              ...
              <Link
                ...
                onClick={() => setMobileMenuOpen(false)}
                className={`flex items-center gap-3 h-11 px-3 rounded-xl text-xs font-bold transition-all ${
                  pathname === '/dashboard/reports' ? 'bg-accent text-white shadow-premium' : 'text-text-primary hover:bg-[#ECECE7]/60'
                }`}
              >
              ...
              <Link
                ...
                onClick={() => setMobileMenuOpen(false)}
                className={`flex items-center gap-3 h-11 px-3 rounded-xl text-xs font-bold transition-all ${
                  pathname === '/dashboard/history' ? 'bg-accent text-white shadow-premium' : 'text-text-primary hover:bg-[#ECECE7]/60'
                }`}
              >
